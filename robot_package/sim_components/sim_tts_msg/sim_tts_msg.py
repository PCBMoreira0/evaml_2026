...
#fotofundo
back = Label(janela)
back.la = PhotoImage(file = (BASE_DIR / 'images/tts_msgbox.png').resolve())
back['image'] = back.la
back.place(x=0,y=0)
...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe(topic=[(sim_base_topic + '/TALK', 1), ])
    client.subscribe(topic=[(robot_base_topic + '/TALK', 1), ])
